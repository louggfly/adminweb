...
def dologin(request):
    try:
        ...
        # 根据登录账号获取登录者信息
        adminUser = Admin.objects.get(username=request.POST['username'])
        # 判断当前用户是否是管理员
        if adminUser.status == 1 or 6:
            # 判断登录密码是否相同
            import hashlib
            md5 = hashlib.md5()
            s = request.POST['pass'] + adminUser.password_salt  # 从表单中获取密码并添加干扰值
            md5.update(s.encode('utf-8'))  # 将要产生md5的子串放进去
            if adminUser.password_hash == md5.hexdigest():  # 获取md5值
                print('登录成功')
